...
// node_modules/.pnpm/@deskthing+types@0.11.18/node_modules/@deskthing/types/dist/apps/appSettings.js
var SETTING_TYPES;
...
// node_modules/.pnpm/@deskthing+types@0.11.18/node_modules/@deskthing/types/dist/deskthing/deskthingTransit.js
...
// node_modules/.pnpm/@deskthing+server@0.11.6/node_modules/@deskthing/server/dist/index.js
...
// server/fortune.ts
var FortuneService = class _FortuneService {
  ...
  static getInstance() {
    if (!_FortuneService.instance) {
      _FortuneService.instance = new _FortuneService();
    }
    return _FortuneService.instance;
  }
  getFilteredFortunes() {
    return fortunes.map((f, i) => ({ fortune: f, originalIndex: i })).filter(
      (item) => this.categoryFilter === "all" || item.fortune.category === this.categoryFilter
    );
  }
  pickNewFortune() {
...
var fortune_default = FortuneService;

// server/index.ts
var start = async () => {
  const fortuneService = fortune_default.getInstance();
  let Data = await DeskThing.getSettings();
  DeskThing.on(DESKTHING_EVENTS.SETTINGS, (newData) => {
    Data = newData.payload;
    if (Data) {
      fortuneService.updateSettings(Data);
    }
  });
  if (!Data?.refreshInterval) {
    setupSettings();
